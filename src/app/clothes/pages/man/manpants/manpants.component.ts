...
@Component({
  selector: 'app-manpants',
  templateUrl: './manpants.component.html',
  styleUrl: './manpants.component.css'
})
export class ManpantsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.clothesService.getClothes().subscribe((clothes) => {
      ...
      this.filteredClothes = clothes.filter((cloth) => cloth.type.includes('man-pants'));
