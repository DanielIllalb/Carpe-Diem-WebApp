...
@Component({
  selector: 'app-womenpants',
  templateUrl: './womenpants.component.html',
  styleUrl: './womenpants.component.css'
})
export class WomenpantsComponent implements OnInit {
  clothes!: Clothes[];
  ...
  ngOnInit(): void {
    this.clothesService.getClothes().subscribe((clothes) => {
      ...
      this.filteredClothes = clothes.filter((cloth) => cloth.type.includes('women-pants'));
